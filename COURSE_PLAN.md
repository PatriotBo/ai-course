...
每节课输出：
1. Markdown 讲义 + 统一风格 HTML 课件；
2. 可运行代码；
3. 一套课后练习/作业文件（Markdown + HTML，避免与课堂练习重复）；
4. 参考资料和持续更新的 HTML 术语表；
5. 进度与课程首页更新。
...
当前状态：已完成 L00-L05，正在进行 Week 2 Lesson 6。
当前课：`Prompt 设计原则：instruction / context / examples / output contract`。
下一节：`Week 2 Lesson 7：结构化输出：JSON Schema / Pydantic`。
